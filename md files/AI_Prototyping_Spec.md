...
* *Use subtle Hopi/Navajo/Pacific Northwest geometric motif SVGs* as section dividers, decorative backgrounds, or tiny border accents.
...
* *Key accessibility note:* For users with `prefers-reduced-motion`, animations are either faded transitions or static.
...
2. As user scrolls, Framer Motion `useScroll` triggers 4 phases:
  1. Image expands from 60% to 100vw, text panel fades and slides left.
  2. When full viewport, a new centered headline appears in bold over a darkened scrim.
  3. “Cinematic hold”: photo holds, HopiDivider SVG animates in at bottom (line draw/cultural motif motion).
  4. Image fades out, next section (“Our Story”) slides up as next entry.
3. On mobile, fallback to static split layout with a simple fade—no scroll-linked animation.
...
*All API calls are mocked in frontend using MSW with realistic seed data and in-memory persistence.*
...
* `GET /api/residents` (paginated, filterable)
* `POST /api/residents`
* `PUT /api/residents/:id`
* `DELETE /api/residents/:id`
* `GET /api/residents/:id/recordings`
* `POST /api/recordings`
* `GET /api/supporters`
* `GET /api/donations` (Donor role filters by supporter)
* `GET /api/impact/public`
* `POST /api/auth/login` (returns mock JWT with role)

*Seed at least 20 residents (placeholder names), 10 supporters, 30 donations, 12 months of impact data.*
...
* **Behavior:**
  ...
  * *On mobile*: fallback to static split, no scroll effect
